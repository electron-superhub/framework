...
import type { NodeExternalsOptions } from "@esho/core/types";
...
const normalizeMatcher = (input: string | RegExp | Matcher): Matcher => {
...
const applyProductionCondition = (exports: PackageJson["exports"]) => {
  if (
    !exports ||
...
export function externals(opts: NodeExternalsOptions): Plugin {
  ...
  const isExplicitInline = (id: string, importer?: string) => {
    ...
    if (
      ...
      return true; // 当模块同时匹配 内联和外部规则时，如果 内联规则 score 高于 外部规则，依然认为是内联模块
    }
  };
  ...
  return {
    ...
    async resolveId(originalId, importer, options) {
      // Skip forced externals
      if (opts.forceExternal?.includes(originalId))
        return {
          id: originalId,
          external: true,
        };
      ...
      if (!(await isValidNodeImport(resolved.id).catch(() => false))) {
        return null; // 无法被 Node.js 实际导入的模块 不能作为外部依赖，直接进行内联
      }
    ...
    async buildEnd() {
      ...
      for (const [pkgName, pkgVersions] of Object.entries(multiVersionPkgs)) {
        ...
        for (const [version, parentPkgs] of versionEntries) {
          // Write each version into node_modules/.esho/{name}@{version}
          await writePackage(pkgName, version, `.esho/${pkgName}@${version}`);
          // Link one version to the top level (for indirect bundle deps)
          await linkPackage(`.esho/${pkgName}@${version}`, `${pkgName}`);
          // Link to parent packages
          for (const parentPkg of parentPkgs) {
            const parentPkgName = parentPkg.replace(/@[^@]+$/, "");
            await (multiVersionPkgs[parentPkgName]
              ? linkPackage(
                  `.esho/${pkgName}@${version}`,
                  `.esho/${parentPkg}/node_modules/${pkgName}`
                )
              : linkPackage(
                  `.esho/${pkgName}@${version}`,
